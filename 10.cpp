...
public:
    ...
    friend std::ostream& operator << (std::ostream& stream,
        const QueueBasedOnBidirectionalCyclicLinkedList < T >& _object);
...
void QueueBasedOnBidirectionalCyclicLinkedList < T > ::InsertAt(int _value, int _index) {
    ...
    QueueBasedOnBidirectionalCyclicLinkedList < int >* temp_obj{
        std::move(this)
    };
    int tmp;
    if (_index <= size)
...
int main() {
    ...
    QueueBasedOnBidirectionalCyclicLinkedList < int > initialization_queue = {
        1,
        2,
        3,
        4,
    ...
    QueueBasedOnBidirectionalCyclicLinkedList < int > copy_queue{
        initialization_queue
    }; // Конструктор кпирования
    QueueBasedOnBidirectionalCyclicLinkedList < int > temp_queue{
        copy_queue
    }; // Конструктор кпирования
    QueueBasedOnBidirectionalCyclicLinkedList < int > move_queue{
        std::move(temp_queue)
